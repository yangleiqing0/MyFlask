...
def to_regist_variables(method, url, data, headers, regist_variable='', regular=''):
    ...
    if regist_variable and regular:
        ...
        if len(regist_variable_value) > 0:
            if Variables.query.filter(Variables.name == regist_variable).count() > 0:
                print('%s 请求时,存在此变量时：' % url,  Variables.query.filter(Variables.name == regist_variable).first())
