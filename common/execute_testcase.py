...
def to_execute_testcase(testcase, is_commit=True):
    ...
    testcase_name, testcase_url, testcase_data = AnalysisParams().analysis_more_params(
        testcase.name, testcase.url, testcase.data, testcase_name=testcase.name)

    testcase_request_header_value = AnalysisParams().analysis_params(
        testcase_request_header.value, is_change='headers', testcase_name=testcase.name)
