...
class TestCases(db.Model):
    ...
    def __repr__(self):
        return "<TestCase:%s,%s,%s,%s,%s,%s,%s,%s,%s,%s >" % (
            self.id, self.name, self.url, self.data, self.method,
            self.group_id, self.request_headers_id, self.timestamp, self.regist_variable,self.regular)

    def to_json(self):
        return dict(id=self.id, name=self.name, url=self.url,
                    data=self.data, regist_variable=self.regist_variable,
                    method=self.method, group_id=self.group_id, regular=self.regular)